...
@ApiStatus.Internal
public record StartSpellEffectPayload(RegistryEntry<SpellEffect> entry) implements CustomPayload {
    ...
    public StartSpellEffectPayload(SpellEffect spellEffect) {
        this(SpellEffects.REGISTRY.getEntry(spellEffect));
    }

    public static StartSpellEffectPayload read(RegistryByteBuf buf) {
        return new StartSpellEffectPayload(SpellEffects.ENTRY_PACKET_CODEC.decode(buf));
    }
